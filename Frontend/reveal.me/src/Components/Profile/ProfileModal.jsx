const ProfileModal = ({handleMatch, popup}) => {
    const person = popup.props.person;
    ...
    return (
        <>
            <label htmlFor={person._id}
                   className="btn bg-darker-pink modal-button ">{person.first_name}</label>
            <input type="checkbox" id={popup.props.person._id} className="modal-toggle"/>
            <label htmlFor={person._id} className="modal cursor-pointer">
                <div className="flex flex-col items-center">
                    {popup}
                    <button
                        id={`matchButton${person._id}`}
                        onClick={() => onButtonPress(person._id)}
