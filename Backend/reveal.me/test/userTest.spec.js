...
const successfulRegister =
  require("../testData/register.json").successfulRegister;
const failRegister = require("../testData/register.json").failRegister;
const correctConfirmPassword =
  require("../testData/forgetPassword.json").correctConfirmPassword;
const falseConfirmPassword =
  require("../testData/forgetPassword.json").falseConfirmPassword;
const passwordTooShort =
  require("../testData/forgetPassword.json").passwordTooShort;
const fullHeaderData = require("../testData/updateData.json").fullHeaderData;
const missingHeaderData =
  require("../testData/updateData.json").missingHeaderData;
  ...
  it("should successfully update user details", (done) => {
    request(baseurl)
      ...
      .end(function (err, res) {
        ...
        expect(res.body.userDetail.gender_interest).to.have.members(
          fullBodyData.gender_interest
        );
        expect(res.body.userDetail.age).to.be.equal(fullBodyData.age);
        expect(res.body.userDetail.profile_picture).to.be.equal(
          fullBodyData.profile_picture
        );
        expect(res.body.userDetail.dob_date).to.be.equal(fullBodyData.dob_date);
        expect(res.body.userDetail.dob_month).to.be.equal(
          fullBodyData.dob_month
        );
        ...
        expect(res.body.userDetail.nationality).to.be.equal(
          fullBodyData.nationality
        );
        expect(res.body.userDetail.occupation).to.be.equal(
          fullBodyData.occupation
        );
        expect(res.body.userDetail.hobbies).to.have.members(
          fullBodyData.hobbies
        );
        expect(res.body.userDetail.languages).to.have.members(
          fullBodyData.languages
        );
        if (err) {
          throw err;
        }
